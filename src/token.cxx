...
std::string_view keyword2String(Keyword keyword)
{
  switch (keyword) {
    ...
    case Keyword::Simple: return "simple";
  }
  BUCKET_UNREACHABLE();
}
...
#define BUCKET_L_BRACKET "\xE2\x9F\xA8"
#define BUCKET_R_BRACKET "\xE2\x9F\xA9"

std::ostream& operator<< (std::ostream& stream, const Token& token)
{
  if (auto identifier = token.getIdentifier()) {
    return stream << BUCKET_L_BRACKET "identifier `" << *identifier
                  << "`" BUCKET_R_BRACKET "\n";
  }
  else if (auto keyword = token.getKeyword()) {
    return stream << BUCKET_L_BRACKET "keyword `" << keyword2String(*keyword)
                  << "`" BUCKET_R_BRACKET "\n";
  }
  else if (auto symbol = token.getSymbol()) {
    return stream << BUCKET_L_BRACKET "symbol `" << symbol2String(*symbol)
                  << "`" BUCKET_R_BRACKET "\n";
  }
  else if (auto integer_literal = token.getIntegerLiteral()) {
    return stream << BUCKET_L_BRACKET "integer `" << *integer_literal
                  << "`" BUCKET_R_BRACKET "\n";
  }
  else if (auto real_literal = token.getRealLiteral()) {
    return stream << BUCKET_L_BRACKET "real `" << *real_literal
                  << "`" BUCKET_R_BRACKET "\n";
  }
  else if (auto string_literal = token.getStringLiteral()) {
    return stream << BUCKET_L_BRACKET "string `" << *string_literal
                  << "`" BUCKET_R_BRACKET "\n";
  }
  else if (auto character_literal = token.getCharacterLiteral()) {
    return stream << BUCKET_L_BRACKET "character `" << *character_literal
                  << "`" BUCKET_R_BRACKET "\n";
  }
  else if (auto boolean_literal = token.getBooleanLiteral()) {
    return stream << BUCKET_L_BRACKET "boolean `"
                  << (*boolean_literal ? "true" : "false")
                  << "`" BUCKET_R_BRACKET "\n";
  }
  else if (token.isEndOfFile()) {
    return stream << BUCKET_L_BRACKET "eof" BUCKET_R_BRACKET "\n";
  }
  BUCKET_UNREACHABLE();
}
