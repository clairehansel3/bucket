...
// at 'iter' in the source code and returns a token.
{
  ...
  switch (*iter) {
        ...
        if (*iter == '/') {
          ...
          while (true) {
            ...
            if (iter == end) {
              ...
              ++++begin_plus_one;
              exception("lexer", "file ends inside multiline comment (depth ",
                depth, ") which starts here:\n", source_file.highlight(begin,
                begin_plus_one));
            }
            else if (*iter == '*') {
              ++iter;
              ...
              if (iter == end) {
                ...
                assert(depth != std::numeric_limits<decltype(depth)>::max());
                ++depth;
              }
            }
          }
    ...
    case '\"':
    {
      ...
      while (iter != end && *iter != '"') {
        if (*iter == '\\') {
          ...
          ++iter;
          if (iter == end)
            exception("lexer", "file ends while inside string which starts here"
              ":\n", source_file.highlight(begin));
          switch (*iter) {
            case 'a':
              string += '\a';
            ...
            default:
              {
                ...
                ++iter;
                exception("lexer", "invalid escape sequence '\\", character,
                  "':\n", source_file.highlight(start_of_escape_sequence,
                  iter));
              }
          }
        }
    ...
    case '\'':
    {
      ...
      ++iter;
      if (iter == end)
        exception("lexer", "file ends while inside character literal which star"
          "ts here:\n", source_file.highlight(begin));
      else if (*iter == '\'') {
        ++iter;
        exception("lexer", "empty character literal:\n", source_file.highlight(
          begin, iter));
      }
      else if (*iter == '\\') {
        ++iter;
        if (iter == end)
          exception("lexer", "file ends while inside character literal which st"
            "arts here:\n", source_file.highlight(begin));
        switch (*iter) {
          case 'a':
            character = '\a';
          ...
          default:
          {
            ...
            ++iter;
            exception("lexer", "invalid escape sequence '\\", character_utf8,
              "':\n", source_file.highlight(start_of_escape_sequence, iter));
          }
        }
      }
      else {
      ...
      if (iter == end)
        ...
        exception("lexer", "extra character appears in character literal:\n",
    ...
    case '_':
    {
      std::string string;
      do {
        // NOTE: if you switch to allowing unicode in strings you should use
        // utf8::append
        #ifdef BUCKET_DEBUG
        string += boost::numeric_cast<char>(*iter);
        #else
        string += static_cast<char>(*iter);
        #endif
        ++iter;
      } while (iter != end
    ...
    case '9':
    {
      ...
      if (*iter == '.') {
        ...
        if (iter != end && *iter == '.') {
          ...
          if (iter != end && (isLetter(*iter) || *iter == '_'))
            exception("lexer", "letter appears in number literal:\n",
              source_file.highlight(iter));
      ...
      if (iter != end && (*iter == 'e' || *iter == 'E')) {
        ...
        if (iter == end)
          exception("lexer", "file ends in real literal starting here:\n",
            source_file.highlight(begin));
        else if (!isDigit(*iter))
          exception("lexer", "expected digit in real literal exponent:\n",
            source_file.highlight(iter));
        do {
          number += static_cast<char>(*iter);
          ++iter;
    ...
    default:
    {
      ...
      exception("lexer", "unidentified character '", character, "' (U+",
        character_hex.str(), ") in source code:\n",
        source_file.highlight(iter));
    }
  }
}
